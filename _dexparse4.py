...
targets={'getBluetoothDeviceBattery','getBluetoothDeviceMode','getBluetoothDeviceVolume','getBluetoothDeviceAncMode'}
    ...
    for ci in range(cls_size):
        try:
            ...
            for _ in range(dm+vm):
                ...
                nm=sstr(struct.unpack_from('<I',d,method_off+midx*8+4)[0])
                if nm in targets: hit.append(nm)
            if hit: print(dn, cname, '->', hit)
        except Exception:
            pass
print('done')
