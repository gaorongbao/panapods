...
type_size = struct.unpack_from('<I', d, 64)[0]
type_off  = struct.unpack_from('<I', d, 68)[0]
method_size = struct.unpack_from('<I', d, 88)[0]
...
def tstr(idx):
    sidx = struct.unpack_from('<I', d, type_off + idx*4)[0]
    return sstr(sidx)

target = 'Lcom/miui/circulate/api/protocol/headset/HeadsetServiceController;'
for ci in range(cls_size):
    ...
    if tstr(struct.unpack_from('<I', d, cd)[0]) == target:
        ...
        for _ in range(dm + vm):
            ...
            m = method_off + midx*8
            names.append(sstr(struct.unpack_from('<I', d, m+4)[0]))
        print('total methods:', len(names))
        for n in names:
            print('  ', n)
        break
